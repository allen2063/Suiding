...
public abstract class PullToRefreshBase<Target extends View> extends LinearLayout
{
    // ===========================================================
    // Constants
    // ===========================================================
    ...
    private int mode = MODE_PULL_DOWN;
    private int curmode;
    ...
    private void initialized(Context context, AttributeSet attrs) {
        ...
        if (array.hasValue(R.styleable.PullToRefresh_mode)) {
            mode = array.getInteger(R.styleable.PullToRefresh_mode, MODE_PULL_DOWN);
        }

        // Refreshable View
        // By passing the attrs, we can add ListView/GridView params via XML
        mTargetView = this.createRefreshableView(context, attrs);
        LayoutParams lpTarget  = new LayoutParams(LayoutParams.FILL_PARENT, 0, 1.0f);
        super.addView(mTargetView,lpTarget);

        LayoutParams lpHeader = new LayoutParams(LayoutParams.FILL_PARENT,LayoutParams.WRAP_CONTENT);
        // Add Loading Views
        if (mode == MODE_PULL_DOWN || mode == MODE_BOTH) {
            mHeaderLayout = new HeaderLayout(context,attrs);
        ...
        if (mode == MODE_PULL_UP || mode == MODE_BOTH) {
            mFooterLayout = new FooterLayout(context,attrs);
        ...
        if (array.hasValue(R.styleable.PullToRefresh_headerTextColor)) {
            final int color = array.getColor(R.styleable.PullToRefresh_headerTextColor, Color.BLACK);
        ...
        if (array.hasValue(R.styleable.PullToRefresh_headerBackground)) {
            this.setBackgroundResource(array.getResourceId(R.styleable.PullToRefresh_headerBackground, Color.WHITE));
        }
            ...
            case MotionEvent.ACTION_MOVE: {
                if (isReadyForPull()) {
                    ...
                    if (yDiff > touchSlop && yDiff > xDiff) {
                        if ((mode & MODE_PULL_DOWN) == MODE_PULL_DOWN  && dy >= 0.0001f
                                && isReadyForPullDown()) {
                            lastMotionY = y;
                            isBeingDragged = true;
                            if (mode == MODE_BOTH) {
                        ...
                        } else if ((mode & MODE_PULL_UP) == MODE_PULL_UP && dy <= 0.0001f
                                && isReadyForPullUp()) {
                            lastMotionY = y;
                            isBeingDragged = true;
                            if (mode == MODE_BOTH) {
    ...
    private boolean pullEvent() {
        ...
        switch (curmode) {
            case MODE_PULL_UP:
                newHeight = Math.round(Math.max(initMotionY - lastMotionY, 0) / FRICTION);
                break;
            case MODE_PULL_DOWN:
            default:
                newHeight = Math.round(Math.min(initMotionY - lastMotionY, 0) / FRICTION);
                break;
        }
     ...
     */
    protected abstract Target createRefreshableView(Context context, AttributeSet attrs);
    ...
    protected final void smoothScrollTo(int y) {
        ...
        if (this.getScrollY() != y) {
            this.smoothrunnable = new SmoothRunnable(Looper.myLooper(),mSmooth, getScrollY(), y);
